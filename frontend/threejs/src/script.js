...
loader.load(
  ...
  function (gltf) {
    ...
    // boxes.push({ mesh, rigidBody });
    carGLTF.children[0].material.wireframe = true;
    const carMesh = new THREE.Mesh(carGLTF.children[0].geometry, carGLTF.children[0].material);
    const carModel = carGLTF.children[0];
    console.log(carModel);
    carMesh.castShadow = true;
    carMesh.receiveShadow = true;
    scene.add(carMesh);
    const carDefaultPostion = new THREE.Vector3(1, 1, 2);
    carMesh.position.set(carDefaultPostion.x, carDefaultPostion.y, carDefaultPostion.z);

    carModel.geometry.computeBoundingBox();
    const geometry = carModel.geometry;

    // // Get bounding box for a simple collider
    // const bbox = geometry.boundingBox;
    // const sizeCollider = new THREE.Vector3(1, 1, 1);
    // bbox.getSize(sizeCollider);

    // // Rapier wants half-extents
    // const halfExtents = { x: sizeCollider.x / 2, y: sizeCollider.y / 2, z: sizeCollider.z / 2 };

    // const rigidBodyDesc = RAPIER.RigidBodyDesc.dynamic().setTranslation(carDefaultPostion.x, carDefaultPostion.y, carDefaultPostion.z);
  ...
  // Create a trimesh collider
  const colliderDesc = RAPIER.ColliderDesc.trimesh(geometry.attributes.position.array , geometry.index.array);//vertices,indices
  world.createCollider(colliderDesc, rigidBody);


    rigidBody.setAngvel({ x: 5, y: 0, z: 0 }, true);

    for (wheels in carMesh.children){


    }

    boxes.push({mesh:carMesh, rigidBody})
...
function animate(now = performance.now()) {
  ...
  for (const { mesh, rigidBody } of boxes) {
