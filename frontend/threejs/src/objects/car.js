...
export class CarObject {
    constructor(scene, world) {
        ...
        loader.load(
            ...
            (gltf) => {
                ...
                const rigidBodyDesc = RAPIER.RigidBodyDesc.dynamic().setTranslation(carMesh.position.x, carMesh.position.y, carMesh.position.z)//.setGravityScale(0); //
        ...
        loader.load(
            ...
            (gltf) => {
                ...
                gltf = gltf.scene
                console.log(gltf)
                var wheelObj = gltf.getObjectByName("wheel")
                console.log(wheelObj)
                wheelObj.scale.set(size, size, size); // scale the object
              

                scene.add(wheelObj)
                wheelObj.position.y = 0.5;


                const wheelGeometry = wheelObj.geometry;


                wheelGeometry.computeBoundingBox();
                const rigidBodyDesc = RAPIER.RigidBodyDesc.dynamic().setTranslation(0, 0.5, 0)//.setGravityScale(0); //
                const rigidBody = world.createRigidBody(rigidBodyDesc);
                const colliderDesc = RAPIER.ColliderDesc.trimesh(wheelGeometry.attributes.position.array, wheelGeometry.index.array); //vertices,indices
                console.log(colliderDesc)
                console.log(rigidBody)
                ...
                this.wheel = { mesh: wheelObj, rigidBody: rigidBody }
                console.log("modified")
    ...
    updateCar() {
        ...
        if (this.isLoaderWheelDone) {
            ...
            const t = this.wheel.rigidBody.translation();
            const r = this.wheel.rigidBody.rotation();

            this.wheel.mesh.position.set(t.x, t.y, t.z);
            this.wheel.mesh.quaternion.set(r.x, r.y, r.z, r.w);
