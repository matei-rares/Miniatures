...
export class CarObject {
    ...
    loadWheels(scene, world) {
        this.loader.load(
            ...
            (gltf) => {
                ...
                this.wheelFL.rigidBody.setTranslation({ x: 0, y: 1, z: 2.6 }, false);

                this.wheelFR = this.wheels[1];
                this.wheelFR.rigidBody.setTranslation({ x: 0, y: 1, z: 1 }, false);

                this.wheelBL = this.wheels[2];
                this.wheelBL.rigidBody.setTranslation({ x: 2, y: 1, z: 2.6 }, false);

                this.wheelBR = this.wheels[3];
                this.wheelBR.rigidBody.setTranslation({ x: 2, y: 1, z: 1 }, false);
    ...
    update() {
        ...
        if(this.isLoaderCarDone && this.isLoaderWheelDone && !this.finalInit){
            ...
            this.world.createImpulseJoint(RAPIER.JointData.revolute(new RAPIER.Vector3(1, 0, 1), new RAPIER.Vector3(1, 1, 0), new RAPIER.Vector3(2, 0, 0)), this.car.rigidBody, this.wheelBR.rigidBody, true)
            this.finalInit = true;
        }
